...
export default function findNextAvailableDaysInWeek(availableDays: Array<string>) {
    ...
    for (let i = 1; i <= 6 - todayIndex; i++) { // فقط روزهای باقی‌مانده از هفته را بررسی می‌کنیم
        const nextDayIndex = todayIndex + i;

        if (nextDayIndex < 7 && availableDayDoctor.includes(nextDayIndex)) { // فقط روزهای این هفته را در نظر بگیریم
