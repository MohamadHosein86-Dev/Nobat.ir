...
export default function Home() {
  ...
  function gotoNextPage(key:string , value:number){
     searchparams.set(key , value);
     setSerchparams(searchparams);
  }
       ...
       <div className=" w-[100%] flex  md:gap-[2.8rem]  ">
          ...
          <div>
            {Array.from({length : totalPages} , (__ , index)=>(
                <button onClick={()=>gotoNextPage("page", index+1)} key={index +1} className=" text-[.85ren] rounded-[50%] w-[2rem] h-[2rem] text-black custom_shadow3 bg-[#fff] flex justify-center items-center" >
                  {index+1}
                </button>
            ))}
          </div>
