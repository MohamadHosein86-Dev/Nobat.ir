...
export default function PageDotor() {
  ...
  const [searcgParams , setSearcgParams] =useSearchParams();
   ...
   const time = doctor?.working_hours.map((res)=> res.day) 
   const day = doctor?.working_hours.map((res)=>(res.day));
   const doctorDay = time?.includes(day); 
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                       ...
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <MoalGetNobat isTodayAcsepting={isTodayAcsepting  }  time={""}/>
                        </Modal.Window>}
                     </Modal>
                    )}
                  </div>
