...
export default function PageDotor() {
  ...
  const isToday = doctor?.working_hours  &&  findNextAvalableDay(workingHours).map((res)=> res.nextDay) ;
  const isTodayAcsepting = isToday?.map((res)=> res);


  const kirDay = doctor?.working_hours?.map((res)=>res.day);
  console.log(isTodayAcsepting ===kirDay?.map((res)=>res));
            ...
            <div className=" ">
                  <div className=" flex gap-[1.5rem] overflow-x-auto mt-[1rem] justify-center">
                    ...
                    doctor?.working_hours.map((res)=>
                      ...
                      <div style={{minWidth: "7rem"}} key={res.day} className={` ${isTodayAcsepting === res.day  ? "bg-slate-500" : "bg-[#f9ffea]"} flex flex-col justify-center font-semibold items-center border-[1px] rounded-[10px] border-[#75950c] text-[#75950c] w-[7rem] h-[7.5rem] bg-[#f9ffea]`}>
                        {res.day}
