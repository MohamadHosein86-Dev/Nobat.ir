...
export default function PageDotor() {
  ...
  const navigate =useNavigate(); 
    ...
    const time11 = doctor?.working_hours.map((res)=> (isTodayAcsepting?.includes(res.day) && res )); 
    const time_slots =  time11?.map((res)=>res?.time_slots).map((res)=>res?.map((res)=>res.from ));
