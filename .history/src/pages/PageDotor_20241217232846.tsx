            ...
            <div className=" ">
                  <div className=" flex gap-[1.5rem] overflow-x-auto mt-[1rem] justify-center">
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                        <Modal.Open openwindowName={`${isTodayAcsepting?.includes(res.day) ? "turns" :""}`}>
                         <div>
                            {isTodayAcsepting?.includes(res.day)}  
                            <button style={{minWidth: "7rem"}} key={res.day} className={` cursor-pointer ${isTodayAcsepting?.includes(res.day)  ? "bg-[#f9ffea]" : "bg-[#eee] text-[#aaa]"  } flex flex-col justify-center font-semibold items-center border-[1px] rounded-[10px] border-[#75950c] text-[#75950c] w-[7rem] h-[7.5rem] bg-[#f9ffea]`}>
                                <span> {res.day} </span>
                                <span> {res.time_slots.map((item)=> (<p key={item.to} className=" text-[.8rem] mt-[.5rem]">از {item.from} تا {item.to}</p>))}  </span>
                            </button>
                         </div>
                        </Modal.Open>
                        <Modal.Window name={`${isTodayAcsepting?.includes(res.day) ? "turns" :""}`}>
                            <>kitr</>
                        </Modal.Window>
                     </Modal>
                    )}
