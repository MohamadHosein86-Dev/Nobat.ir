...
export default function PageDotor() {
  ...
  const isTodayAcsepting = doctor?.working_hours  && 
  findNextAvalableDay(workingHours).map((res)=> res.nextDay) === doctor.working_hours.map((res)=> res.day);
            ...
            <div className=" ">
                  <div className=" flex gap-[1.5rem] overflow-x-auto mt-[1rem] justify-center">
                    ...
                    doctor?.working_hours.map((res)=>
                      ...
                      <div style={{minWidth: "7rem"}} key={res.day} className={` ${isTodayAcsepting ? "bg-slate-500" : "bg-[#f9ffea]"} flex flex-col justify-center font-semibold items-center border-[1px] rounded-[10px] border-[#75950c] text-[#75950c] w-[7rem] h-[7.5rem] bg-[#f9ffea]`}>
                        {res.day}
