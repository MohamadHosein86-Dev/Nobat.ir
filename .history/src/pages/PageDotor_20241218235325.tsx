...
export default function PageDotor() {
  ...
  const [searcgParams , setSearcgParams] =useSearchParams();
   ...
   const timeOfAvalblableDay = doctor?.working_hours.map((res)=> (isTodayAcsepting?.includes(res.day) && res.time_slots.map((res)=>(res.from , res.to)))); 
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                       ...
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <MoalGetNobat isTodayAcsepting={isTodayAcsepting  }  time={timeOfAvalblableDay}/>
                        </Modal.Window>}
                     </Modal>
                    )}
                  </div>
