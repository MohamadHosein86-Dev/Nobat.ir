...
export default function Home() {
  ...
  return (
    <section className="max-w-[68rem] mx-auto  px-[1.54rem] py-[1rem] md:px-[4rem] md:py-[2rem] mt-[4rem]">
    <h1>{}</h1>
       <div className=" w-[100%] flex gap-[2.8rem] ">
           <div>
              <ul className=" mr-[1.2rem] w-[11rem]">
                {specialties.map((res)=> <li key={res} className=" font-semibold text-[.9rem] cursor-pointer hover:text-[#363636] list-disc mb-[.8rem] text-[#363636ab] hover:font-semibold">{res}</li>)}
              </ul>
           </div>
           <div>
                <ul className=" flex flex-wrap gap-[1rem]">{doctors?.map((res)=>

                   <li onClick={()=>handleFilter(res.specialty)} className=" border-[1px] border-solid border-[#ececec] rounded-[.25rem] flex flex-col justify-between w-[22rem] px-[1rem] py-[.5rem] h-[11.5rem]" key={res.id}>
