...
export default function Home() {
  const {doctors  ,isLoading , totalPages , goToNextPage}=useDoctors();
       ...
       <div className=" w-[100%] flex  md:gap-[2.8rem]  ">
          ...
          {totalPages > 1 && <div>
            {Array.from({length : totalPages} , (__ , index)=>(
                <button onClick={()=>goToNextPage(index+1)} key={index +1} className=" text-[.85ren] rounded-[50%] w-[2rem] h-[2rem] text-black custom_shadow3 bg-[#fff] flex justify-center items-center" >
                  {index+1}
                </button>
            ))}
          </div>}
