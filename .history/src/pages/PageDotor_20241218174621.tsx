...
export default function PageDotor() {
  ...
  const navigate =useNavigate(); 
    ...
    const time_slots = doctor?.working_hours.map((res)=> (isTodayAcsepting?.includes(res.day) && res )); 
    console.log(time_slots?.map((res)=>res?.time_slots).map((res)=>res?.map((res)=>res.to , res.from)));
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                       ...
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <MoalGetNobat isTodayAcsepting={isTodayAcsepting  }  time_slots={time_slots} />
                        </Modal.Window>}
                     </Modal>
                    )}
                  </div>
