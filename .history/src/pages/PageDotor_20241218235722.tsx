...
export default function PageDotor() {
  ...
  const navigate =useNavigate(); 
    ...
    const from = doctor?.working_hours.map((res)=> (isTodayAcsepting?.includes(res.day) && res.time_slots.map((res)=>res.from))); 
    const to = doctor?.working_hours.map((res)=> (isTodayAcsepting?.includes(res.day) && res.time_slots.map((res)=>res.to))); 
                     ...
                     <Modal>
                        <Modal.Open openwindowName="s">
                         <div className=" relative">
                            ...
                            <button  style={{minWidth: "7rem"}} key={res.day} 
                       ...
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <MoalGetNobat isTodayAcsepting={isTodayAcsepting  }  from={from} to={to} />
                        </Modal.Window>}
                     </Modal>
                    )}
                  </div>
