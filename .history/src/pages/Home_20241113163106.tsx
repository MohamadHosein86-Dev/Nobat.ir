  ...
  return (
    <section className=" w-full">
     <div className="  max-w-[68rem] mx-auto  px-[1.5rem] py-[1rem] md:px-[4rem] md:py-[2rem]">
       <h1 className=" md:text-right md:mb-[3.4rem] md:mt-[-.8rem] text-center text-[1.3rem] sm:text-[1.3rem] font-medium text-[#044c6a] mb-[1.3rem] mt-[1.4rem]"> { (city || specialties ) &&` لیست آدرس و تلفن ، پزشکان ${specialty == null ? "" : specialty}  ${city === null ? "" :city} ` }</h1> 
       <div className=" w-[100%] flex  md:gap-[2.8rem]  ">
           <div className="hidden md:block">
              <ul className=" mr-[1.2rem] w-[11rem] ">
                {specialties.map((res)=> <li onClick={()=>{handleFilter(res);updatePrams("specialty" , res) }} key={res} className=" font-semibold text-[.9rem] cursor-pointer hover:text-[#363636] list-disc mb-[.8rem] text-[#363636ab] hover:font-semibold">{res}</li>)}
              </ul>
           </div>
           <div className=" w-full">
             
                 <ul className=" w-full flex flex-wrap gap-[1rem]">{doctors?.map((res)=>
                   <li  className=" cursor-pointer w-[100%] xl:w-[22rem] border-b-[1px] border-b-[#ececec] md:border-[1px] md:border-solid md:border-[#ececec] rounded-[.25rem] flex flex-col justify-between  px-[1rem] py-[.5rem] h-[11.5rem]" key={res.id}>
