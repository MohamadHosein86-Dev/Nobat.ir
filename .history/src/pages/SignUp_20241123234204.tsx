...
export default function SignUp() {
    ...
    async function handleLogin(x:React.FormEvent<HTMLFormElement>) {
        ...
        signUp({email , password},{onSuccess:(data)=>{
          setPassword("");
          setEmail("");
          if(data){
            data.user?.role === "authenticated" ? navigate("/") : ""
          }
        }});
    }
  ...
  return (
    <section className=" flex justify-center  w-[100%] h-[100%]">
        <form onSubmit={handleLogin} className=" flex flex-col w-[24rem] h-[100vh] justify-center  ">
             ...
             <button className=" mt-[.8rem] mx-auto bg-[#cccc] flex items-center justify-center rounded-[4px] w-[7rem] text-[#ffff] h-[2.3rem] pb-[.4rem]"> ورود به حساب </button>
        </form>
    </section>
  )
}
