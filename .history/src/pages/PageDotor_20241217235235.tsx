 ...
 return (
    <section className="">
      ...
      <div className="  max-w-[68rem] mx-auto  px-[1.5rem]  py-[1rem] flex-col   md:px-[4rem] md:py-[2rem]">
       ...
       <div className="flex-col relative  lg:flex-row flex justify-between  ">
            <div className=" ">
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                        <Modal.Open openwindowName="s">
                         <div >
                            {!isTodayAcsepting?.includes(res.day) && <span className=" bg-[red] absolute top-[-.4rem] rounded-[.5rem] right-[.2rem]">نوبت دهی ندارد</span>}  
                            <button onClick={()=>setShowModal((res)=>res!)} style={{minWidth: "7rem"}} key={res.day} className={` cursor-pointer ${isTodayAcsepting?.includes(res.day)  ? "bg-[#f9ffea]" : "bg-[#eee] text-[#aaa]"  } flex flex-col justify-center font-semibold items-center border-[1px] rounded-[10px] border-[#75950c] text-[#75950c] w-[7rem] h-[7.5rem] bg-[#f9ffea]`}>
                                <span> {res.day} </span>
                                <span> {res.time_slots.map((item)=> (<p key={item.to} className=" text-[.8rem] mt-[.5rem]">از {item.from} تا {item.to}</p>))}  </span>
                            </button>
                         </div>
                        </Modal.Open>
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <>kitr</>
                        </Modal.Window>}
                     </Modal>
                    )}
