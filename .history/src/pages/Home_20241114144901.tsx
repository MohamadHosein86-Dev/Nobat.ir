...
export default function Home() {
  ...
  function handleFilterLi(key:string , value:string | number) {  
  ...
  return (
    <section className=" w-full">
     <div className="  max-w-[68rem] mx-auto  px-[1.5rem] py-[1rem] md:px-[4rem] md:py-[2rem]">
       <h1 className=" md:text-right md:mb-[3.4rem] md:mt-[-.8rem] text-center text-[1.3rem] sm:text-[1.3rem] font-medium text-[#044c6a] mb-[1.3rem] mt-[1.4rem]"> { (city || specialties ) !== null ? ` لیست آدرس و تلفن ، پزشکان ${specialty }  ${city} ` : `` }</h1> 
       <div className=" w-[100%] flex  md:gap-[2.8rem]  ">
          ...
          <div>
            {Array.from({length : totalPages} , (__ , index)=>(
                <button onClick={()=>handleFilterLi("page", index+1)} key={index +1} className=" text-[.85ren] rounded-[50%] w-[2rem] h-[2rem] text-black custom_shadow3 bg-[#fff] flex justify-center items-center" >
                  {index+1}
                </button>
            ))}
          </div>
