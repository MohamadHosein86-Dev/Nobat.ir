...
export default function PageDotor() {
    ...
    const time_slots =  doctor?.working_hours?.filter((res)=> isTodayAcsepting?.includes(res.day)).flatMap((res)=>res.time_slots);
                    ...
                    doctor?.working_hours.map((res)=>
                     <Modal>
                       ...
                       {isTodayAcsepting?.includes(res.day) && <Modal.Window name="s">
                            <MoalGetNobat isTodayAcsepting={ res.day }  time_slots={ isTodayAcsepting?.includes(res.day)?.flatMap((res)=>res.time_slots) } />
                        </Modal.Window>}
                     </Modal>
                    )}
                  </div>
