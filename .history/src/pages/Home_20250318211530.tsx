  ...
  return (
    <section className=" w-full">
     <div className="  max-w-[68rem] mx-auto  px-[1.5rem] py-[1rem] md:px-[4rem] md:py-[2rem]">
       <h1 className=" md:text-right md:mb-[3.4rem] md:mt-[-.8rem] text-center text-[1.3rem] sm:text-[1.3rem] font-medium text-[#044c6a] mt-[1.4rem]"> { (city || specialty ) !== null ? ` لیست آدرس و تلفن ، پزشکان ${specialty !== null ? specialty : "" }  ${city !== null ? city : ""} ` : `` }</h1> 
       <div className=" w-[100%] flex  md:mt-[-2.5rem]  md:gap-[2.8rem]  ">
           <div className="hidden md:block ">
              <ul className=" mr-[1.2rem] w-[11rem] ">
                {specialties.map((res)=> <li onClick={()=>{handleFilterLi("specialty" , res) }} key={res} className=" font-normal text-[.9rem] cursor-pointer hover:text-[#363636] list-disc mb-[.8rem] text-[#363636ab] hover:font-semibold">{res}</li>)}
              </ul>
           </div>
