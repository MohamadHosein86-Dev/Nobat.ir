...
export default function Home() {
  ...
  return (
    ...
    <h1 className=" text-[1.5rem] font-semibold text-[#044c6a] mb-[3.5rem] mt-[-.7rem]">{` لیست آدرس و تلفن ، پزشکان ${specialty == null ? "" : specialty} ، ${city === null ? "" :city} ` }</h1>
       <div className=" w-[100%] flex gap-[2.8rem] ">
           ...
           <div>
                <ul className=" flex flex-wrap gap-[1rem]">{doctors?.map((res)=>
                   ...
                   <li  className=" xl:w-[100%] border-[1px] border-solid border-[#ececec] rounded-[.25rem] flex flex-col justify-between w-[22rem] px-[1rem] py-[.5rem] h-[11.5rem]" key={res.id}>
