...
export default function useDoctors() {
    ...
    //search
    const tehranSearch = data?.filter((doctor)=> doctor.address.replace(/،$/,"")[0].includes("تهران"));
    console.log(tehranSearch);
    
    const searchDoctors = doctorsFilter?.filter((item)=> search == "" ? "" : search.split(" ").every((word)=>(item.name.includes(word) || item.specialty.includes(word)))  )
