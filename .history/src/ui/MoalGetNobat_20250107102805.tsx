...
export default function MoalGetNobat({isTodayAcsepting , time_slots }:typeInput) {
    ...
    function handleAccpet() {
        ...
        dispatch(addNobatDaayDoctors({from , to , name}));
    ...
    function handleHuorsNobat(From:string , To:string , name:string ,index:number) {
      setFrom(From);
      setTo(To);
      setName(name)
      setSelected(index);
    ...
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              const isNobat =nobatDay.flatMap((res)=>res.to).includes(res.to) && doctorsNobat.flatMap((res)=>res)
              console.log(doctorsNobat.flatMap((res)=>res));
