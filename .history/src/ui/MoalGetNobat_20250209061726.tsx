...
export default function MoalGetNobat({isDayNobat , time_slots , isTodayAcsepting }:typeInput) {
    ...
    function handleAccpet() {
        dispatch(addNobatDaayDoctors({from  ,to  ,nobatDay ,isDayNobat ,name , image , specialty ,id ,phone }));
        setShowBtn(selected);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              const isNobat = NobatDoctors.some(
                (resp) => resp.to === res.to && resp.name === doctor?.name
              );
