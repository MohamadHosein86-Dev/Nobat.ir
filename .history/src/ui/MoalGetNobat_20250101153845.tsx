...
export default function MoalGetNobat({isTodayAcsepting , time_slots }:typeInput) {
    ...
    console.log(useSelector((res:TypeNobatDoctors)=>res.nobatDoctors) , doctorsNobat.includes(doctor));
    ...
    const [isNobat , setIsNobat]=useState(false);
    ...
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              return(
                  <div key={index} className={` cursor-pointer ${isDesable || !doctorsNobat.includes(doctor) ? "bg-[#ababab] text-[#343a40] " : ""} justify-center rounded-[.3rem] border-[#f5f5f5] border-[1px] p-[.5rem] flex items-center gap-[.8rem] w-[10rem]`}>
                    <label  htmlFor=""> از  {res.from} تا {res.to} </label>
                    <input checked={isSelect}  className={`${isDesable || !doctorsNobat.includes(doctor) ? "hidden" : ""}  chelckBox`}   onChange={handleHuorsNobat} type="checkbox" />
                  </div>
