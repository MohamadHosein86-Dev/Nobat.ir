...
export default function MenuFilterCity({data , plasehoder ,CloseModal}:typeMneu) {
  ...
  function updatePrams(key , value) {
  ...
  return (
    <div className=" bg-white px-[1.2rem] w-[100%] md:pt-[2rem] p-b[2rem] md:w-[26rem] before:content-['']  before:absolute before:top-[20px] before:w-[50px] before:left-[50%] before:translate-x-[-50%] before:h-[3px] before:bg-[#ccc] before:rounded-[40px]  ">
        ...
        <ul className=" overflow-y-scroll h-[60rem]  menu">
          {data[0] !==  "قلب و عروق" && dataCityes.map((res)=> <li  onClick={()=>{handleFilter1(res) ; updatePrams("specialties",res) ;CloseModal()}} className=" cursor-pointer border-b-[1px] text-center py-[1rem] " key={res}>{res}</li>)}
          {data[0] !== "تهران"  &&   dataCityes.map((res)=> <li  onClick={()=>{handleFilter2(res); updatePrams("city",res) ;CloseModal()}} className=" cursor-pointer border-b-[1px] text-center py-[1rem] " key={res}>{res}</li>)} 
        </ul>
    </div>
  )
}
