...
export default function MoalGetNobat({isTodayAcsepting , time_slots }:typeInput) {
    ...
    const [showBtn , setShowBtn] = useState<number | string>("");
    const [from , setFrom] = useState("");
    const [to , setTo] =useState("");
    // Nobat & doctor
    
    
    const isShowBtn = selected === showBtn ;
  ...
  return (
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      {  <button  onClick={handleAccpet} className=" mt-[2rem] text-[#ffff] text-sm font-bold rounded-[1rem] text-center bg-[#66de66] p-[1rem]" >ثبت نوبت از ساعت  تا {from} {to} {doctor?.name}  </button>  }   
