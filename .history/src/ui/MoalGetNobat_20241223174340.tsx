  ...
  return (
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
        ...
        <div className="z-0 mt-[1rem] flex gap-[1rem]">
          ...
          <div  key={index} className={` ${isToday && time > res.from ? "bg-[#ababab] text-[#343a40] " : ""}  cursor-pointer rounded-sm border-[#f5f5f5] border-[1px] p-[.5rem] flex items-center gap-[.8rem] w-[10rem]`}>
              <label  htmlFor=""> از  {res.from} تا {res.to} </label>
              <input  className="chelckBox"  disabled={ isToday && time > res.from}  onChange={(x)=>setCheacked(x.target.checked)} type="checkbox" />
          </div>
          )}
