...
*/
  ...
  return (
    <div className="  max-w-[68rem] mx-auto  px-[1.5rem]  py-[1rem] md:px-[4rem] md:py-[2rem]">
      {specialties.includes(search) && <button className="pb-[.3rem] w-[21rem] h-[3.2rem] mx-auto mb-[2rem] text-[#ffff] bg-[#0092ab] border-[#fff] rounded-[9px] border-[1px] custom_shadow4 text-[.95rem] flex items-center justify-center  font-medium " onClick={()=>{}}> لیست پزشکان {search}</button>  }
      ...
      {specialties.includes(search) && <button>{search}</button>  }
    </div>
  )
}
