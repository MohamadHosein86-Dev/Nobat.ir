  ...
  const {user}=useUser();
   ...
   const [showFulNobat, setShowFulNobat] = useState(Array(NobatDoctors.length).fill(false));
    const handleClick = (index:number) => {
       const newShowFulNobat = [...showFulNobat];
        newShowFulNobat[index] = !newShowFulNobat[index]; setShowFulNobat(newShowFulNobat);
    }
  return (
    <section>
        ...
        <div className=" flex flex-col  w-[100%] ">
          {NobatDoctors.map((resp , index)=>{         
            ...
            return(
                <div onClick={()=>handleClick(index)} className={` ${showFulNobat[index] && "hidden"} flex gap-[2rem] cursor-pointer pb-[1.2rem] border-b-[1px]  w-[100%] `}  >
