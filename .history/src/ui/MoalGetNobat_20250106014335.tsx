...
export default function MoalGetNobat({isTodayAcsepting , time_slots }:typeInput) {
    ...
    console.log( doctorsNobat.flatMap((res)=>res.working_hours).map((res)=>res.time_slots).map((res)=>res) + " فرار نکن کوصمادر");
    ...
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              return(
                  <div  className={` cursor-pointer ${isDesable  ||  doctorsNobat.flatMap((res)=>res.working_hours).map((res)=>res.time_slots).map((res)=>res.to).includes(res.to) ? "bg-[#ababab] text-[#343a40] " : ""} justify-center rounded-[.3rem] border-[#f5f5f5] border-[1px] p-[.5rem] flex items-center gap-[.8rem] w-[10rem]`}>
                    <label  htmlFor=""> از  {res.from} تا {res.to} </label>
                    <input checked={isSelect}  className={`${isDesable || doctorsNobat.flatMap((res)=>res.working_hours).map((res)=>res.time_slots).map((res)=>res.to).includes(res.to) ? "hidden" : ""}  chelckBox`}   onChange={()=>handleHuorsNobat(res.from ,res.to ,index)} type="checkbox" />
