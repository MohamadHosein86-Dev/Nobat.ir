...
export default function MoalGetNobat({isTodayAcsepting , time_slots  }:typeInput) {
    ...
    function handleAccpet() {
        dispatch(addNobatDaayDoctors({from  ,to ,name , image , specialty}));
        setShowBtn(selected);
    }
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              return(
                    ...
                    <div key={}   className={` cursor-pointer ${ isDesable || isNobat ? "bg-[#ababab] text-[#343a40] " : ""} justify-center rounded-[.3rem] border-[#f5f5f5] border-[1px] p-[.5rem] flex items-center gap-[.8rem] w-[10rem]`}>
