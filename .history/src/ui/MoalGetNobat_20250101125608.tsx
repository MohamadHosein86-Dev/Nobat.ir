...
export default function MoalGetNobat({isTodayAcsepting , time_slots }:typeInput) {
    ...
    /* const {isNobat1} = useSelector((res:TypeNobatDoctors)=>res.nobatDoctors) */
    console.log(isNobat1);
  ...
  return (
    <div className=" w-[26rem] top-0 h-lvh mb-[9.5rem] rounded-sm flex flex-col relative bg-[#fff]">
      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
        ...
        {showBtn && !isNobat && <button onClick={()=>setIsNobat(true)} className=" mt-[2rem] text-[#ffff] text-sm font-bold rounded-[1rem] text-center bg-[#66de66] p-[1rem]" >ثبت نوبت از ساعت  تا {from} {to} {doctor?.name}  </button>}   
