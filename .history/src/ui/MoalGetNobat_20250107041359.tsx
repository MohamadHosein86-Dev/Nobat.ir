      ...
      <div className="z-0 mt-[1rem] flex gap-[1rem]">
            {time_slots?.map((res , index)=>{
              ...
              return(
                  ...
                  <div>
                    <div key={index}   className={` w cursor-pointer ${ isDesable || showBtn === index  ? "bg-[#ababab] text-[#343a40] " : ""} justify-center rounded-[.3rem] border-[#f5f5f5] border-[1px] p-[.5rem] flex items-center gap-[.8rem] w-[10rem]`}>
                        <label  htmlFor=""> از  {res.from} تا {res.to} </label>
                        <input checked={isSelected}  className={`${isDesable || showBtn === index  ? "hidden" : ""}  chelckBox`}   onChange={()=>handleHuorsNobat(res.from ,res.to ,index )} type="checkbox" />
                        <input type="button"  onClick={handleAccpet} className=" mt-[2rem] text-[#ffff] text-sm font-bold rounded-[1rem] text-center bg-[#66de66] p-[.2rem]" >ثبت نوبت از ساعت  تا {from} {to} {doctor?.name}  </input> 
                    </div>
