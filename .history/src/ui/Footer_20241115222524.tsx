...
export default function Footer() {
  ...
  function handleLink() {
    if(pathname == "/"){
      ...
      searcgParams.set("city" , doctor?.specialty);
  ...
  return (
    <section className=" w-[100%] mt-[2.5rem]  bg-footer-500  pb-[2rem]">
       <div className=" w-full bg-[#e5f9ff] pb-[1.3rem] pt-[1.3rem]">
          ...
          <Link onClick={handleLink} to={""} className={` pb-[.4rem]  custom_shadow2  text-[#598a98] font-sans  "w-[100%]"  h-[2.3rem]  flex justify-center items-center rounded-[6px]  bg-[#fafafa]" to={""} `}>
            {pathname == "/" ?  "پرسش و پاسخ" : `لیست پزشکان ${doctor?.specialty} ، ${cityAdress}` }
          </Link>
          </div>
       </div> 
